...
def load_s3_df(bucket_name, filepath):
    ...
    for i, obj in enumerate(files):
        ...
        if fl[-1] == 'x':
            ...
            tabular_data = pd.read_excel(obj1['Body'].read())
        else:
            pass
...
class OPV_CV_im_dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, im_dict, label_df, index):
        super(OPV_CV_im_dataset).__init__()
        self.im_dict = {}
...
class OPV_CV_total_dataset(torch.utils.data.Dataset):
    ...
    with k-fold cross-validation functions.
    """
    def __init__(self, im_dict, im_labels, m2py_dict, m2py_labels, tabular_data, index):
        super(OPV_CV_total_dataset).__init__()
        self.im_dict = {}
        self.im_labels = im_labels
        
        self.m2py_dict = {}
        self.m2py_labels = m2py_labels
        ...
        for key, value in im_dict.items():
            if key in index:
                self.im_dict[new_indx] = im_dict[key]
                self.m2py_dict[new_indx] = m2py_dict[key]
                self.tabular_data_dict[new_indx] = tabular_data.iloc[key]
                new_indx += 1
            else:
                pass
    ...
    def __getitem__(self, key):
        ...
        m2py = self.m2py_dict[key]
        self.m2py_tensor = self.convert_im_to_tensors(m2py)
        
        tab = self.tab_data.iloc[key]
        self.tab_tensor = self.convert_label_to_tensors(tab)
        
        #only need to use labels from one dataset, since they
        #should be the same for all 3
        label_df = self.m2py_labels.iloc[key]
        label_df = label_df.drop(['Anneal_time', 'Anneal_temp', 'Substrate', 'Device'])
        self.label_tensor = self.convert_label_to_tensors(label_df)
        
        return self.im_tensor, self.m2py_tensor, self.tab_tensor, self.label_tensor
...
def CV_OPV_total_fit(model, train_loader, test_loader, criterion, lr, epochs):
    """
    This function takes in a model and hyperparameters and performs a single
    fit for a fold of data in k-fold cross-validation. This function returns
    only the results and losses as a dictionary
    ...
    for epoch in range(epochs):
        [pce_train_loss, voc_train_loss, jsc_train_loss, ff_train_loss] = train.train_OPV_total_model(model = model, training_data_set = train_loader, criterion = criterion, optimizer = optimizer)
        ...
        test_losses, test_accs, test_r2s = test.eval_OPV_total_model(model = model,
                                                                   test_data_set = test_loader,
                                                                   criterion = criterion)
        pce_test_epoch_losses.append(test_losses[0])
        voc_test_epoch_losses.append(test_losses[1])
